...
public class AsyncServer {
	...
	private boolean addMe() {
		synchronized (mServers) {
			...
			if (current != null) {
				// Log.e(LOGTAG,
				// "****AsyncServer already running on this thread.****");
				return false;
			}
	...
	private static void run(final AsyncServer server, final Selector selector, final PriorityQueue<Scheduled> queue, final boolean keepRunning) {
		...
		while (true) {
			...
			// see if we keep looping, this must be in a synchronized block
			// since the queue is accessed.
			synchronized (server) {
	...
	private static void runLoop(final AsyncServer server, final Selector selector, final PriorityQueue<Scheduled> queue, final boolean keepRunning)
			throws IOException {
		...
		synchronized (server) {
			...
			if (readyNow == 0) {
				// if there is nothing to select now, make sure we don't have an
				// empty key set
				// which means it would be time to turn this thread off.
				if (selector.keys().size() == 0 && !keepRunning && wait == QUEUE_EMPTY) {
					// Log.i(LOGTAG, "Shutting down. keys: " +
					// selector.keys().size() + " keepRunning: " + keepRunning);
					return;
				}
			} else {
				needsSelect = false;
